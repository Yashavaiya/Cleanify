...
export class InvoiceComponent implements OnInit {
  ...
  createInvoiceProcess(FormValue: any) {
    FormValue.customer_Id = this.customerId;
    FormValue.service = this.servicesArray;
    FormValue.setting_Id = this.settingData[0]._id;
    console.log("FormValue:", FormValue)

    if (this.createInvoice.valid && FormValue.customer_Id && FormValue.service) {
      this.authService.createInvoice(FormValue).subscribe((response: any) => {
        console.log("response :", response);
        if (response.status == "success") {
          this.showMessage("Invoice Create successfully", 1000);
          setTimeout(() => {
  ...
  isFieldInvalid(fieldName: string) {
    const control = this.createStoreForm.get(fieldName);
  ...
  deleteServiceGroup(index: number) {
    ...
    this.createInvoice.get('total_amount').setValue(totalAmountWithTax);
    this.createInvoice.get('tax_amount').setValue(taxAmount);
  ...
  onChangeHour(index: number) {
    const amount = (this.items.at(index).get('rate').value || 0) *
      (this.items.at(index).get('hour').value || 0);
    this.items.at(index).get('amount').setValue(amount);
    this.servicesArray[index].hour = this.items.at(index).get('hour').value
    this.servicesArray[index].rate = this.items.at(index).get('rate').value
    this.servicesArray[index].amount = this.items.at(index).get('amount').value
